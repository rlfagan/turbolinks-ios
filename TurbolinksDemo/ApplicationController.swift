...
class ApplicationController: UINavigationController {
    fileprivate let URL = Foundation.URL(string: "http://localhost:9292")!
    ...
    fileprivate func presentVisitableForSession(_ session: Session, URL: Foundation.URL, action: Action? = .Advance) {
      let visitable = DemoViewController(url: URL)
      if action == nil || action == .Advance {
